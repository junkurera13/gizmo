...
class DeviceEncodingTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_cinema_skins_cover_fill_above_the_caption_band(self):
        ...
        self.assertIn('<p id="status" aria-live="polite"></p>', cinema_html)
        self.assertIn("height:var(--caption-band);background:#000", cinema_css)
        self.assertNotIn("background:#05111f", cinema_css)
        self.assertNotIn('id="progress"', cinema_html)
        self.assertNotIn("#progress{", cinema_css)
        self.assertNotIn("background:#f5a3b9", cinema_css)
        self.assertIn('class="device-talk"', cinema_html)
        self.assertIn('id="talk"', cinema_html)
        self.assertNotIn("<svg", cinema_html)
        self.assertNotIn("Hold the pink button on the device to talk", cinema_html)
        self.assertNotIn("Hold the pink button to talk ·", cinema_html)
        self.assertNotIn("Ask something. See where it takes us.", cinema_html)
        self.assertNotIn("What are you curious about?", cinema_html)
        self.assertNotIn("You can interrupt anytime", cinema_html)
        self.assertNotIn('placeholder=', cinema_html)
        self.assertNotIn('id="hint"', cinema_html)
        self.assertNotIn('class="hint"', cinema_html)
...
class FriendSocketTests(unittest.TestCase):
    ...
    def test_cinema_page_and_static_still_load(self):
        with (
            ...
            self.assertNotIn('id="progress"', page.text)
            self.assertNotIn('id="ptt"', page.text)
            self.assertIn('class="device-talk"', page.text)
            self.assertIn("device-reference-ptt-pressed.png", page.text)
            self.assertIn("dataset.ptt", script.text)
            self.assertIn("mouseleave", script.text)
            self.assertIn("touchstart", script.text)
            self.assertIn("mousedown", script.text)
            self.assertNotIn("Ask something. See where it takes us.", page.text)
            self.assertNotIn("Ask something. See where it takes us.", script.text)
            self.assertNotIn("Where does that take your curiosity?", script.text)
            self.assertNotIn("What are you curious about?", page.text)
            self.assertNotIn("Hold the pink button", page.text)
            self.assertNotIn("You can interrupt anytime", page.text)
            self.assertNotIn("I’m listening. Where should we go from here?", script.text)
            self.assertNotIn("Listening…", script.text)
            self.assertNotIn(".placeholder", script.text)
            self.assertIn('<p id="status" aria-live="polite"></p>', page.text)
            self.assertIn("The picture connection dropped", script.text)
            self.assertIn("Connection closed", script.text)
