...
macOS runtime uses the bundled FFmpeg binary for real AVFoundation microphone
input and AudioToolbox speaker output.
...
hold captures only real 24 kHz mono PCM. It does not activate a camera or send a
visual frame. Incoming PCM plays through the selected AudioToolbox output and is
printed only as a byte count.

The microphone and speaker index options select the AVFoundation/AudioToolbox
devices. The defaults use input index 0 and the system's default output; use `--speaker-index`
...
- PTT owns only microphone capture. Camera input waits for a separately designed
  See interaction and is not exposed as a command-line input.
