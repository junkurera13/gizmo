...
def header(ref,value,x,y,pins,labels,horizontal=False,left=False):
    ...
    for i,label in enumerate(labels):
        if horizontal:text(label,x+i*2.54,y-2.6,.75)
        else:text(label,x+(-2 if left else 2),y+i*2.54,.80,align='right' if left else 'left')
    text(ref+' '+value,x+(xx/2 if horizontal else (-2 if left else 2)),y-4.3 if horizontal else y-2.8,.95,align='center' if horizontal else ('right' if left else 'left'))
    return fp
...
text('FROM PROTECTED OUT / ISOLATED 5V',19,59,.72)
...
for ref,label,x,y,n in [('TP1','3V3',35,19,'3V3'),('TP2','GND',40,19,'GND')]:
    ...
    add(fp,ref,label,x,y,{'1':n},'TestPoint_1.5mm');text(ref+' '+label,x,y-1.6,.7)
...
for s,x in [('BAT+',12),('GND',14.54),('5V',17.08)]:text(s,x,63,.7,p.B_SilkS)
...
def schematic_component(c,x,y,kind='header'):
    ...
    if kind=='module':
        ...
        for i,(num,nn) in enumerate(ns.items()):
            ...
            pn=nn.replace('DISP_','').replace('_UNUSED','');typ='power_out' if nn=='3V3' else ('power_in' if nn in ['5V','GND'] else 'bidirectional')
        ...
        for i,(num,nn) in enumerate(ns.items()):pins.append((num,nn or 'NC',nn,-5.08,-i*5.08,0,'passive'))
        box=(0,2.54,22.86,-(n-1)*5.08-2.54)
    a,b,d,e=box
    graphics=f'(rectangle (start {a} {b}) (end {d} {e}) (stroke (width .254) (type default)) (fill (type background)))'
    pintext=''.join(f'(pin {typ} line (at {px} {py} {angle}) (length 5.08) (name {q(pn)} (effects (font (size 1.0 1.0)))) (number {q(num)} (effects (font (size 1 1)))))' for num,pn,nn,px,py,angle,typ in pins)
    ...
    for num,pn,nn,px,py,angle,typ in pins:
        ...
        if nn:
            ...
            labels.append(f'(label {q(nn)} (at {end} {yy} {0 if angle==0 else 180}) (effects (font (size 1 1)) (justify left bottom)) (uuid {q(uid(ref+"label"+num))}))')
...
note('POWER: BAT+ is protected battery OUT+ for sensing. 5V is isolated boost/USB VUSB.',20,122,1.05)
note('XIAO underside BAT/USB/debug pads intentionally NOT contacted. Do not parallel chargers.',20,128,1.05)
note('SWITCHES: each remote UP / DOWN / SEL closes to 3V3. PTT closes to GND.',245,285,1.05)
note('Ladder: idle 0V; SEL 1.32V; DOWN 2.24V; UP 3.3V. BAT divider = 2.0.',20,284,1.05)
