...
- Speaker: **Adafruit STEMMA Speaker** (PID **3885**), PAM8302A Class-D plus a
  ...
  black = GND). Signal on **D9** (GPIO8) as the physically proven 9-bit /
  62.5 kHz LEDC PWM profile. Keep the assembled direct jumper unchanged for the
  current software-only regression test; an analog filter is a later contingency,
  not a prerequisite for this flash. Replaces the earlier Youmile MAX98357 I2S
  breadboard amp (BCLK/LRC/DIN). On-board trim pot is analog gain; firmware
  volume still scales PCM before PWM.
...
| D9 | 8 | STEMMA Speaker IN (LEDC PWM) | GPIO8 → assembled Adafruit 3885 SIGNAL (white) jumper. Also Sense microSD MISO; SD CS is held HIGH |
...
- Speaker PWM lives on D9 (GPIO8, LEDC channel 4 / timer 2) at the proven
  9-bit / 62.5 kHz profile. PDM RX lives on I2S_NUM_0 (GPIO41/42). PWM is
  detached when idle so D9 sits LOW (no carrier).
...
| Speaker and amplifier | Adafruit STEMMA Speaker 3885 on D9; flash and physically compare the restored 9-bit profile before considering any filter or wiring change; analog IN + onboard PAM8302A, not I2S |
