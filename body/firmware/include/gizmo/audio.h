...
// as 9-bit / 62.5 kHz LEDC PWM on D9 (Adafruit STEMMA analog IN). PDM TX is
// I2S0-only, so the speaker cannot share that peripheral with the mic. update()
// pumps samples from loop(). PWM stops when idle so D9 sits low.
namespace gizmo {
