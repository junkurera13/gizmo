...
All four services run in Singapore: `gizmo-brain`, `memobase`, `postgres`, and `redis`. Only the brain has a public domain. Device/data routes and the device WebSocket require `Authorization: Bearer <GIZMO_DEVICE_TOKEN>`. The OddityOS shell, its static assets, moment sessions, and the Cinema workbench at `/cinema` are public so `oddware.xyz/gizmo/oddity` can embed them — there is no access code. Public sessions use isolated random identities, enforce same-origin sockets, and have separate daily spend caps.

Pushes to `main` deploy automatically via Railway's GitHub source (declared in `.railway/railway.ts`): `gizmo-brain` builds from the repo root Dockerfile on every green `main` push, and `memobase` only when the push touches `deploy/memobase/` (service watch patterns). Both deployment triggers have `checkSuites` enabled, so Railway waits for the `tests` workflow to pass before deploying — a red merge does not reach production. Manual fallback: `npx @railway/cli up --service gizmo-brain` / `--service memobase` from the repo root (works for any local checkout, no GitHub needed). The `RAILWAY_TOKEN` Actions secret is retained for manual CLI use but no workflow step consumes it.
