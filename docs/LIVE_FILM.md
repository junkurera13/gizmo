...
Friend owns the `/ws` session. Cinema is one capability that session can start and stop: the silent visual director chooses `film` when the glass should **move**. That includes a hard explanation (how something works, what happens when, a process) and a story's opening or an actual move to a new setting. Easy talk — greetings, jokes, feelings — stays words. The kid does not need to say film, movie, or cinema. Friend then reuses `cinema/runtime.py` (H3 Max Director) plus `DeviceFilmPlayer` to play 320×240 held-cue MJPEG and the original PCM on the body. PTT, Select, settings, stills, memory, and ordinary talk stay on `GizmoSession`. After the film ends or is interrupted, conversation returns to Friend. The old `GIZMO_DIRECTOR_DEVICE` whole-session swap is gone.

Fal `minimax/h3-max/image-to-video` still→short-clip (`session._start_motion`, leftover `animate`, StoryRun `_render_motion`) is **not** the product path for explanations or story motion. It remains only for a bare “make it move” on a still already on the glass. Do not add new callers.
...
The initial wait is too long to call this instant. Next performance work should measure planning, per-beat TTS, signaling, first generated frame, and browser presentation separately before choosing caching, a warmer connection, or streamed first-beat narration. Avoid masking latency with a fake pre-generated answer. Physical acceptance should use a continuous screen/audio/button recording from the cofounder.
