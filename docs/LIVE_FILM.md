...
The old Fal `minimax/h3-max/image-to-video` still→short-clip path is **not** used for explanations. Moving explanations are Cinema only. A leftover short clip remains **only** for a bare “make it move” on a still already on the glass (`session._animate` / `_start_motion`). StoryRun `_render_motion` is leftover and unused. Do not add new callers.
