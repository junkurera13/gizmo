...
`cinema/device.py` buffers five seconds of incoming video, cover-crops it into a 320×192 picture above a 48-pixel caption band, produces ESP-compatible 8 fps MJPEG through the existing Show store, and pairs each segment with the corresponding original 24 kHz PCM. The browser `/cinema` skin and Oddity cinema-on-device stage use the same layout: the film cover-fills the picture area, and captions sit in the reserved bottom strip. Every cue is adaptively JPEG-compressed to at most 256 KiB, with individual frames held below 7 KiB, so the next segment can download and each picture can decode within the playback interval. Motion acknowledgement is required before `go`; failures stop the speech instead of letting it drift. Select dismisses, PTT stops the film and returns to Friend listening, and brightness/volume retain their existing Friend controls.
