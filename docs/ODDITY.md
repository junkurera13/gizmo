...
Open [OddityOS 1](http://127.0.0.1:43148/oddity). No separate frontend build or Node server is required. `gizmo` loads the existing root `.env`; all provider keys stay on the server. Local `/oddity` is the same chromeless moment player the site embeds, with no code gate. Cinema from Oddity and `/cinema` itself also open without an access code.
